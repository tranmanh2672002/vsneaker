<template>
    <v-form ref="form" @submit.prevent="addProduct" v-if="modalType === 'Product'">
        <v-card class="pa-2">
            <v-card-title headline class="font-weight-bold mt-2">Add Product</v-card-title>
            <v-card-text>
    ...
    <v-form v-else-if="modalType === 'Account'" ref="form" @submit.prevent="addAccount">
        <v-card class="pa-2">
    ...
    <v-form v-else-if="modalType === 'Discount'" ref="form" @submit.prevent="applyDiscount">
        <v-card class="pa-2">
            <v-card-title headline class="font-weight-bold mt-2">Apply Discount</v-card-title>
            <v-card-text>
                <v-text-field density="comfortable" v-model="discountName" clearable label="Discount Name"
                    :rules="rules"></v-text-field>
                <v-text-field density="comfortable" type="number" v-model="saleOff" clearable label="Sale-off"
                    :rules="rules"></v-text-field>
                <v-row>
                    <v-col cols="6">
                        <v-text-field density="comfortable" type="date" v-model="startAt" label="Start at"
                            :rules="rules"></v-text-field>
                    </v-col>
                    <v-col cols="6">
                        <v-text-field density="comfortable" type="date" v-model="endAt" label="End at"
                            :rules="rules"></v-text-field>
                    </v-col>
                </v-row>

                <v-textarea density="comfortable" v-model="descriptionDiscount" label="Description" clearable rows="3"
                    row-height="30" :rules="rules"></v-textarea>
    ...
    <v-form v-else-if="modalType === 'Voucher'" ref="form" @submit.prevent="addVoucher">
        <v-card class="pa-2">
            <v-card-title headline class="font-weight-bold mt-2">Add Voucher</v-card-title>
            <v-card-text>
                <v-select density="comfortable" v-model="addedVoucher" clearable label="Add voucher" :items="listDiscount"
                    :rules="rules" class="mt-2"></v-select>
                <v-autocomplete v-model="productsDiscount" :items="listProducts" chips closable-chips
                    color="blue-grey-lighten-2" item-title="Product_name" item-value="Product_name" label="Select" multiple
                    class="selector-discount-product">
                    <template v-slot:chip="{ props, item }">
                        <v-chip v-bind="props" :prepend-avatar="item.raw.Product_IMG"
                            :text="item.raw.Product_name"></v-chip>
                    </template>

                    <template v-slot:item="{ props, item }">
                        <v-list-item v-bind="props" :prepend-avatar="item?.raw?.Product_IMG"
                            :title="item?.raw?.Product_name" :subtitle="item?.raw?.Sku"></v-list-item>
    ...
    <v-form v-else ref="form" @submit.prevent="deleteDiscount">
        <v-card class="pa-2">
            <v-card-title headline class="font-weight-bold mt-2">Delete Discount</v-card-title>
            <v-card-text>
                <v-select density="comfortable" v-model="deletedDiscount" clearable label="Delete Discount"
                    :items="listDiscount" :rules="rules" class="mt-2"></v-select>
                <v-btn color="black" type="submit" class="mb-3 d-flex align-center float-right"
...
function getProductDiscountId () {
    productsDiscount.value.forEach(currProduct => {
        let productDiscount = listProducts.value.find(product => product.Product_name == currProduct)
...
if (modalType === "DeleteDiscount" || modalType === "Voucher") {
    ...
    data.forEach((discount) => {
        listDiscount.value.push(`${discount.Discount_name}: ${discount.Description}`);
    })
}
...
async function getDiscountId (event) {
    const data = await getAllDiscount();
    const discountId = data.find(discount => `${discount.Discount_name}: ${discount.Description}` == event)
    return discountId;
}
